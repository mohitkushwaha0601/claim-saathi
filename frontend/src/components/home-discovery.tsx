  ...
  return (
    <section aria-labelledby="discovery-heading" className="mb-12 grid gap-8 rounded-[18px] bg-ink p-5 sm:mb-16 sm:p-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start lg:p-10">
      ...
      <div className="grid gap-4">
        <div><p className="text-xs font-bold tracking-[0.14em] text-gold uppercase">{t("rolesEyebrow")}</p><h2 className="mt-2 text-2xl font-bold tracking-[-0.025em] text-white">{t("rolesTitle")}</h2></div>
...
function DiscoveryContent({ item, t }: { item: ServiceRegistryItem; t: ReturnType<typeof useTranslations> }) {
  ...
  return <><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-3"><span aria-hidden="true" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-brand-soft text-lg font-bold text-brand">{SERVICE_ICONS[item.id] ?? "•"}</span><h2 className="pt-1 font-bold text-ink group-hover:text-brand">{t(`items.${item.titleKey}.title`)}</h2></div>{item.availability === "INFORMATIONAL_PREVIEW" ? <span className="shrink-0 rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-muted">{t("preview")}</span> : <span aria-hidden="true" className="text-brand">→</span>}</div><p className="mt-2 text-sm leading-6 text-muted">{t(descriptionKey)}</p>{ctaKey ? <span className="mt-3 inline-flex text-sm font-bold text-brand">{t(ctaKey)}</span> : null}</>;
}
